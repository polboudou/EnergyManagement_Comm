...
B_TMIN = 20             # Min boiler temperature (degrees celcius)
B_TMAX = 22             # Max boiler temperature (degree celcius)
...
# In roder to test the algorithm, we simulate the power excess (P_PV - P_nc)
...
for h, p_x in enumerate(excess['P_PV - P_nc (kW)']):
    ...
    T_B1 = boiler_states[h][1][TEMP] - p_B1 * (Dt/60) / C_B
    T_B2 = boiler_states[h][2][TEMP] - p_B2 * (Dt/60) / C_B
    print('T_B1:', T_B1, 'and T_B2:', T_B2)
